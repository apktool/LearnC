...
int main(int argc, char* argv[]){
	...
	for(int i=0;i<5;i++){
		...
		int flag;
		flag=connect(listenfd[i],(struct sockaddr*)&client,sizeof(client));
		assert(flag!=-1);

		struct sockaddr_in localaddr;
		socklen_t addrlen=sizeof(localaddr);
		flag=getsockname(listenfd[i],(struct sockaddr*)&localaddr,&addrlen);
		assert(flag!=-1);
	...
	while(fgets(sendbuf,sizeof(sendbuf),stdin)!=NULL){
		writen(listenfd[0],sendbuf,sizeof(sendbuf));
		int ret=readline(listenfd[0],recvbuf,sizeof(recvbuf));
		assert(ret!=-1);
		if(ret==0){
			perror("client close!");
			break;
